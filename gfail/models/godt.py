...
def godt2008(
    ...
    FS = cohesion / (uwt * thick * np.sin(slopestack * (np.pi / 180.0))) + np.tan(
        friction * (np.pi / 180.0)
    ) / np.tan(slopestack * (np.pi / 180.0))
    FS[FS < fsthresh] = fsthresh

    # Compute critical acceleration, in g
    # This gives ac in g, equations that multiply by g give ac in m/s2
    Ac = (FS - 1) * np.sin(slopestack * (np.pi / 180.0)).astype(float)
    Ac[Ac < acthresh] = acthresh

    # Get PGA in g (PGA is %g in ShakeMap, convert to g)
    PGA = np.repeat(pga.getData()[:, :, np.newaxis] / 100.0, 7, axis=2).astype(float)
    if "PGV" in displmodel:  # Load in PGV also, in cm/sec
        PGV = np.repeat(pgv.getData()[:, :, np.newaxis], 7, axis=2).astype(float)
    else:
        PGV = None

    if uncertfile is not None:
        stdpga = np.repeat(uncertpga.getData()[:, :, np.newaxis], 7, axis=2).astype(
            float
        )
        stdpgv = np.repeat(uncertpgv.getData()[:, :, np.newaxis], 7, axis=2).astype(
            float
        )
        # estimate PGA +- 1std
    ...
    if uncertfile is not None:
        PROBmin = Dnmin.copy()
        PROBmin[PROBmin <= dnthresh] = 0.0
        PROBmin[PROBmin > dnthresh] = 1.0
        PROBmin = np.sum(PROBmin, axis=2)
        PROBmax = Dnmax.copy()
        PROBmax[PROBmax <= dnthresh] = 0.0
        PROBmax[PROBmax > dnthresh] = 1.0
    ...
    if uncertfile is not None:
        PROBmin[PROBmin == 1.0] = 0.01
        PROBmin[PROBmin == 2.0] = 0.10
        PROBmin[PROBmin == 3.0] = 0.30
        PROBmin[PROBmin == 4.0] = 0.50
        PROBmin[PROBmin == 5.0] = 0.70
        PROBmin[PROBmin == 6.0] = 0.90
        PROBmin[PROBmin == 7.0] = 0.99
        PROBmax[PROBmax == 1.0] = 0.01
        PROBmax[PROBmax == 2.0] = 0.10
        PROBmax[PROBmax == 3.0] = 0.30
        PROBmax[PROBmax == 4.0] = 0.50
        PROBmax[PROBmax == 5.0] = 0.70
        PROBmax[PROBmax == 6.0] = 0.90
        PROBmax[PROBmax == 7.0] = 0.99

    if slopemin is not None:
        PROB[slopestack[:, :, 6] <= slopemin] = 0.0
        # uncert too
        if uncertfile is not None:
            PROBmin[slopestack[:, :, 6] <= slopemin] = 0.0
            PROBmax[slopestack[:, :, 6] <= slopemin] = 0.0
