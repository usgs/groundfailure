...
def test_logisticmodel():
    modelLQ = {
        "TestModelLQ": {
            ...
            "interpolations": {"vs30": "nearest", "cti1": "linear"},
            "terms": {
                "b1": "log((pga/100.0)*(power(MW,2.)))",
                "b2": "cti1",
                "b3": "log(vs30)",
            },
            "coefficients": {"b0": 15.0, "b1": 2.0, "b2": 0.3, "b3": -4.0},
        }
    }
    ...
    targetLS = np.array(
        [
            [0.61358336819225268, 0.99999969213372109],
    ...
    targetLQ = np.array(
        [
            [0.5803309852347005, 0.27771418649141888],
...
def test_validateTerms():
    data = {
        "b1": "np.nan_to_num(self.layerdict['friction'].getSlice("
...
def test_checkTerm():
    ...
    assert (
        term == "self.layerdict['precip'].getSlice(rowstart, rowend, "
        "colstart, colend, name='precip')"
    )
    assert tterm == ""
