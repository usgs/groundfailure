...
def test_viewdb(tmpdir):
    ...
    try:
        ...
        pathcmd = "gfail --set-default-paths -db %s" % os.path.join(
            datadir, "testevents.db"
        )
        ...
        runcmd5 = "viewdb -s --summaryplotfile %s" % os.path.join(
            tmpdir.name, "test.png"
        )
        rc5, so5, se5 = get_command_output(runcmd5)

        runcmd6 = "viewdb -s --csvfile %s" % os.path.join(tmpdir.name, "test.csv")
        rc6, so6, se6 = get_command_output(runcmd6)

        # Test that everything ran
        np.testing.assert_equal(True, rc1, "%s did not run successfully" % runcmd1)
        np.testing.assert_equal(True, rc2, "%s did not run successfully" % runcmd2)
        np.testing.assert_equal(True, rc3, "%s did not run successfully" % runcmd3)
        np.testing.assert_equal(True, rc4, "%s did not run successfully" % runcmd4)
        np.testing.assert_equal(True, rc5, "%s did not run successfully" % runcmd5)

        # Make sure figure was created
        np.testing.assert_equal(
            True, os.path.isfile(os.path.join(tmpdir.name, "test_overall.png"))
        )
