...
def test_godt2008():
    ...
    maplayers1 = godt2008(
        shakefile, config, displmodel="J_PGA", uncertfile=uncertfile, bounds=bounds
    )
    maplayers2 = godt2008(shakefile, config, displmodel="J_PGA_M", saveinputs=True)
    ...
    np.testing.assert_allclose(
        maplayers1["model"]["grid"].getData(),
        np.array([[0.0, 0.1], [0.9, 0.0]]),
        atol=0.01,
    )
    np.testing.assert_allclose(
        maplayers1["modelmin"]["grid"].getData(), np.array([[0.0, 0.01], [0.9, 0.0]])
    )
    np.testing.assert_allclose(
        maplayers1["modelmax"]["grid"].getData(), np.array([[0.0, 0.7], [0.99, 0.0]])
    )

    # Testing J_PGA_M with Uncertainty - Model Type 2
    np.testing.assert_allclose(
        maplayers2["model"]["grid"].getData(),
        np.array([[0.0, 0.01], [0.9, 0.0]]),
        atol=0.01,
    )

    # Testing RS_PGA_M with Uncertainty - Model Type 3
    np.testing.assert_allclose(
        maplayers3["model"]["grid"].getData(),
        np.array([[0.0, 0.5], [0.99, 0.0]]),
        atol=0.01,
    )

    # Testing RS_PGA_PGV with Uncertainty - Model Type 4
    np.testing.assert_allclose(
        maplayers4["model"]["grid"].getData(),
        np.array([[0.0, 0.7], [0.99, 0.0]]),
        atol=0.01,
    )
    np.testing.assert_allclose(
        maplayers4["modelmin"]["grid"].getData(), np.array([[0.0, 0.1], [0.9, 0.0]])
    )
    np.testing.assert_allclose(
        maplayers4["modelmax"]["grid"].getData(), np.array([[0.0, 0.99], [0.99, 0.0]])
    )
    np.testing.assert_allclose(
        maplayers4["pgvmin"]["grid"].getData(),
    ...
    maplayers6 = godt2008(shakefile, config, bounds=bounds, uncertfile=uncertfile2)

    # Testing with changed config
    np.testing.assert_allclose(
        maplayers6["model"]["grid"].getData(),
        np.array([[0.0, 0.01], [0.9, 0.0]]),
        atol=0.01,
    )
