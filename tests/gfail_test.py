...
def test_zhu2015(tmpdir):
    ...
    try:
        ...
        with open(default_file, "a") as f:
            f.write(
                "popfile = %s" % os.path.join(datadir, "loma_prieta/lspop2016_lp.flt")
            )
        ...
        # Run model
        runcmd = "gfail %s/test_conf %s --gis --hdf5 -ext" % (datadir, shakegrid)
        ...
        target_file = os.path.join(
            datadir, "loma_prieta", "targets", "19891018000415_zhu_2015_model.tif"
        )
...
def misc():
    ...
    gridurl = "https://earthquake.usgs.gov/product/shakemap/nc216859/atlas/1594161215615/download/grid.xml"
    test1 = isURL(gridurl)
