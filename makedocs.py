...
def main(args):
    ...
    sphinx_cmd = 'sphinx-apidoc -o %s -f -e -l -d 12 -H %s -A "%s" ' "-V %s %s" % (
        API_DIR,
        PACKAGE,
        AUTHORS,
        verstr,
    ...
    if not res:
        raise Exception(
            "Could not build GroundFailure API documentation" ' - error "%s".' % stderr
        )
    ...
    # --------------------------------------------
    clean_cmd = (
        "sed -i '' -e 's/ module//g' `find %s/*.rst -type f "
        "-maxdepth 0 -print`" % API_DIR
    )
    res, stdout, stderr = get_command_output(clean_cmd)
    clean_cmd = (
        "sed -i '' -e 's/ package//g' `find %s/*.rst -type f "
        "-maxdepth 0 -print`" % API_DIR
    )
    res, stdout, stderr = get_command_output(clean_cmd)
    clean_cmd = (
        "sed -i '' -e '/Subpackages/d' `find %s/*.rst -type f "
        "-maxdepth 0 -print`" % API_DIR
    )
    res, stdout, stderr = get_command_output(clean_cmd)
    clean_cmd = (
        "sed -i '' -e '/-.*-/d' `find %s/*.rst -type f " "-maxdepth 0 -print`" % API_DIR
    )
    res, stdout, stderr = get_command_output(clean_cmd)
    ...
    if not res:
        raise Exception(
            "Could not build HTML for API documentation. - "
